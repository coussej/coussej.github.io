...
Another possible downside is the file based configuration management, but maybe we can improve on that in the future.
...
Another very important one: it's open. If you want to gather data from some obscure system, you can write a small collector for it that pushed the data over HTTP to InfluxDB. This allows you to interface with nearly everything. 

Finally, I think it's all pretty slick. Both InfluxDB and Grafana leverage modern technologies that result in a nice end-user experience. Querying data is fast, dashboards can be easily created and shared within the company, and using the system requires nothing more than a recent browser. The easy installation (just a few commands) allows for a reproducible environment. 
